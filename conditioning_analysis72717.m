...
% trials = [52 56 70 71 75 76

% trials = [59 61 63 65 66 67 68 69 70 71 75 76 89 90 91 95 97 98 100 101 104 106 108 109 110 111 112 ];
...
% Set variable names
contrastLog=cell2mat(contrastLog);
acousticLog=cell2mat(acousticLog);
msiLog=cell2mat(msiLog);
...
vMSIlog=msiLog(acousticLog==0);
%MSI1
vContrastsMSI1 = vContrasts(vMSIlog==1);
...
%MSI2
vContrastsMSI2 = vContrasts(vMSIlog==2);
...
msMean=[ms0mean;ms25mean];
msVar=[ms0var;ms25var];
msCheck_speed = check_speed(acousticLog==1);
ms0btwnMean = nanmean(msCheck_speed(msContrasts==0));

msMSIlog=msiLog(acousticLog==1);
%MSI1
msContrastsMSI1 = msContrasts(msMSIlog==1);
...
%MSI2
msContrastsMSI2 = msContrasts(msMSIlog==2);
...
ms25varMSI2 = nanvar(msDeltaSpeed(msContrastsMSI2==1));
...
% save trial data
save((sprintf('Conditioning Trial %d',trial)),'marker','average_before_speeds','average_after_speeds',...
    'average_btwn_speeds','delta_speed','vContrasts','vDeltaSpeed','msContrasts',...
    'msDeltaSpeed','vMean','vVar','msMean','msVar','MSIn','ms0btwnMean',...
    'vContrastsMSI1','vContrastsMSI2','msContrastsMSI1','msContrastsMSI2',...
    'vMeanMSI1','vMeanMSI2','msMeanMSI1','msMeanMSI2',...
    'vVarMSI1','vVarMSI2','msVarMSI1','msVarMSI2','-append');
clearvars -except trials y m
